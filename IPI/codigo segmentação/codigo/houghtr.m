% Detetar linhas utilizando a transformada de Hough

function houghtr(x)
im=imread(x);
im = medfilt2(im);
f=edge(im,'canny');
figure,imshow(f),title('Image after applying Canny Filter');pause;
[h,theta,rho]=hough1(f,0.5);
imshow(theta,rho,h,[],'notruesize'),axis on,axis normal;
xlabel('\theta'),ylabel('\rho'); pause;
[r,c]=houghpeaks1(h,10);
hold on;
plot(theta(c),rho(r),'linestyle','none','marker','s','color','w'); pause;
lines=houghlines1(f,theta,rho,r,c);
figure,imshow(f),hold on;
for k=1:length(lines)
    xy=[lines(k).point1;lines(k).point2];
    plot(xy(:,2),xy(:,1),'Linewidth',4,'Color',[.6 .6 .6]); pause;
end 
end
